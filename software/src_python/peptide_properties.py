...
def main():
    ...
    df = (
        ...
        .with_columns(
            ...
            pl.col("variantKey")
            .str.slice(0, pl.col("variantKey").str.len_chars() - pl.col("variantKey").str.len_chars() % 3)
            .alias("ntTrimmed")
        )
        .with_columns(
        ...
        .with_columns(
            pl.col("variantKey").alias("nSeqPeptide"),
            pl.col("variantKey").str.len_chars().alias("ntLengthPeptide"),
            pl.col("aaSeqPeptide").str.len_chars().alias("aaLengthPeptide"),
            # QC flags (vectorized, no Biopython)
            (pl.col("variantKey").str.len_chars() % 3 != 0).alias("hasTrailingNucleotides"),
